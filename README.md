...
- **Neovim AI stack on NixOS**: resolved. `mcp-hub` uses mcphub.nvim's bundled build (`build = "bundled_build.lua"` + `use_bundled_binary = true`). `claude-code` and `claude-code-acp` are pulled from `nixpkgs-unstable` via an overlay in `flake.nix` to stay current — update with `nix flake update`. A shell wrapper aliases `claude-code-acp` to `claude-agent-acp` (the name codecompanion expects).
